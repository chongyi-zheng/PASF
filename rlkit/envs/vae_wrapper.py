...
class VAEWrappedEnv(ProxyEnv, MultitaskEnv):
    ...
    def sample_goals(self, batch_size):
        ...
        if self._goal_sampling_mode == 'custom_goal_sampler':
            ...
            idx = np.repeat(self.env_idx, batch_size, axis=0)
            ...
            idx = np.repeat(self.env_idx, batch_size, axis=0)
        elif self._goal_sampling_mode == 'random_domain_vae_prior':
            goals = {}
            latent_goals = self._sample_vae_prior(batch_size)
            n = self.env_idx.shape[1]
            i = np.random.randint(n, size=(batch_size,))
            idx = np.eye(n)[i].reshape(batch_size, -1)
        else:
            raise RuntimeError("Invalid: {}".format(self._goal_sampling_mode))

        if self._decode_goals:
            latents = np.concatenate([latent_goals, idx], axis=-1)
            decoded_goals = self._decode(latents)
        else:
            decoded_goals = None
