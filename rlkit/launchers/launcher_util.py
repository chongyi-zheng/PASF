...
def setup_logger(
    ...
    first_time = log_dir is None and not checkpoint
    if first_time:
        log_dir = create_log_dir(exp_prefix, **create_log_dir_kwargs)
        logger.log(f"New log directory: {osp.abspath(log_dir)}")
    elif checkpoint:
        checkpoint_dir = osp.abspath(checkpoint_dir)
        logger.log(f"Checkpoint found: {checkpoint_dir}")
        log_dir = checkpoint_dir

        variant_log_path = osp.join(checkpoint_dir, variant_log_file)
