...
class Logger(object):
    ...
    def save_extra_data(self, data, file_name='extra_data.pkl', mode='joblib'):
        ...
        if mode == 'joblib':
            ...
            np.savez(file_name, **data)  # savez_compressed is much slower, but savez uses a lot of disk space
        else:
    ...
    def atomic_replace(self, file_names):
        ...
        if self._snapshot_dir:
            ...
            for file_name in file_names:
                ...
                os.replace(
                    temp_file_name,  # new checkpoint
                    temp_file_name.replace('.tmp', '')  # last checkpoint
                )
