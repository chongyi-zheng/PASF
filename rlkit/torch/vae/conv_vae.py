...
class FairVAE(GaussianLatentVAE):
    def __init__(
            self,
            representation_size,
            architecture,
        ...
        self.decoder = decoder_class(
            **deconv_args,
            fc_input_size=representation_size + n_envs,
            init_w=init_w,
            output_activation=decoder_output_activation,
            paddings=np.zeros(len(deconv_args['kernel_sizes']), dtype=np.int64),
            hidden_init=hidden_init,
    ...
    def forward(self, input):
        latent_distribution_params = self.encode(input[0])
        latents = self.reparameterize(latent_distribution_params)
        latents = torch.cat([latents, input[1]], dim=-1)
