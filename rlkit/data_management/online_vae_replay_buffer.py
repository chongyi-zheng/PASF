...
class OnlineVaeRelabelingBuffer(SharedObsDictRelabelingBuffer):
    ...
    def add_decoded_vae_goals_to_path(self, path):
        ...
        idxs = np.repeat(self.env_idx, desired_goals.shape[0], axis=0)
        latents = np.concatenate([desired_goals, idxs], axis=-1)
        desired_decoded_goals = self.env._decode(latents)
    ...
    def get_snapshot(self):
        ...
        snapshot.update(
            vae=self.vae,
            vae_sample_priorities=self._vae_sample_priorities,
            vae_sample_probs=self._vae_sample_probs,
            skew=self.skew,
            epoch=self.epoch,
        )
    ...
    def load_from_snapshot(self, snapshot):
        ...
        self._vae_sample_probs = snapshot['vae_sample_probs']
        self.skew = snapshot['skew']
    ...
    def get_batch(self, idxs):
        ...
        next_obs = normalize_image(self._next_obs[self.decoded_obs_key][idxs])
        return dict(
    ...
    def vae_prob(self, next_vae_obs, indices, **kwargs):
        return compute_p_x_np_to_np(  # compute q^{G}(s)
            self.vae,
            next_vae_obs,
            power=self.power,
            idxs=self.env_idx,
