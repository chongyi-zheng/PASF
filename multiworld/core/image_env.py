...
from multiworld.core.background_iterator import BackgroundIterator
...
class ImageEnv(ProxyEnv, MultitaskEnv):
    def __init__(
            ...
            construct_presampled_goals=False,
            recompute_reward=True
    ):

        """
        :param wrapped_env:
        :param env_background: domain background in image_env Wrapper
        :param imsize:
        :param init_camera:
        :param transpose:
        :param grayscale:
        ...
        self.construct_presampled_goals = construct_presampled_goals
        ...
        self.background_iter = self._load_background_iter(env_background)
    ...
    def reset(self):
        ...
        if self.num_goals_presampled > 0:
            ...
            if self.construct_presampled_goals:
                ...
                # dummy get_image to prevent empty image goal
                self._wrapped_env.get_image(
                    width=self.imsize,
                    height=self.imsize)
                goal['image_desired_goal'] = self._get_flat_img()
            ...
            # This is use mainly for debugging or useless pre-sampling goals.
    ...
    def _get_flat_img(self, background=True):
        image_obs = self._wrapped_env.get_image(
            width=self.imsize,
            height=self.imsize
        )
        background_img = self.background_iter.render()
        if background and (background_img is not None):
            mask = self._wrapped_env.get_image(
                width=self.imsize,
                height=self.imsize,
                segmentation=True
            )[:, :, :-1]
            mask = mask < -0.5
            image_obs = mask * background_img + (1 - mask) * image_obs
    ...
    def _load_background_iter(self, para):
        return BackgroundIterator(self.image_shape, para)

    def render_desired_goal_image(self, state):
        env_state = self.wrapped_env.get_env_state()
        self.wrapped_env.set_to_goal(state)
        # dummy get_image to prevent empty image goal
        self._wrapped_env.get_image(
            width=self.imsize,
            height=self.imsize)
