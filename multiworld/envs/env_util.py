...
def create_stats_ordered_dict(
    ...
    if isinstance(discount, float):
        # discount_sum = (1 - \gamma) \sum_{t = 0}^{T - 2} \gamma^t dist_t + \gamma^{T - 1} dist_{T - 1}
        discount_factors = discount ** np.arange(len(data))
        stats[name + ' Discount Sum'] = np.sum((1 - discount) * discount_factors[:-1] * data[:-1]) + \
                                        discount_factors[-1] * data[-1]

        # normalized_discount_sum = \frac{ \sum_{t = 0}^{T - 1} \gamma^t dist_t }{ \sum_{t = 0}^{T - 1} \gamma^t }
        stats[name + ' Normalized Discount Sum'] = np.sum(discount_factors * data) / np.sum(discount_factors)
