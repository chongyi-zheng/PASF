...
def register_dg_sawyer_envs():
    ...
    for i in range(9):
        tag = 'Domain' + str(i)
        register(
            id='SawyerDoorHookResetFreeEnv'+tag+'-v1',
            entry_point='multiworld.envs.mujoco.sawyer_xyz'
            ...
            kwargs=dict(
                ...
                max_angle=.83,
                xml_path='sawyer_xyz/sawyer_door_pull_hook/'+tag+'.xml',
                reward_type='angle_diff_and_hand_distance',
                reset_free=True,
            )
        )
        register(
            id='SawyerDoorHookEnv'+tag+'-v1',
            entry_point='multiworld.envs.mujoco.sawyer_xyz'
                        '.sawyer_door_hook:SawyerDoorHookEnv',
            tags={
                'author': 'beining',
