...
class MujocoEnv(gym.Env):
    ...
    def render(self, mode='human', width=DEFAULT_SIZE, height=DEFAULT_SIZE, segmentation=False):

        if mode == 'rgb_array':
            self._get_viewer(mode).render(width, height, segmentation=segmentation)
            data = self._get_viewer(mode).read_pixels(width, height, depth=False, segmentation=segmentation)
            ...
            data = self._get_viewer(mode).read_pixels(width, height, depth=True, segmentation=segmentation)[1]
            # original image is upside-down, so flip it
            return data[::-1, :]
        elif mode == 'human':
    ...
    def get_image(self, width=84, height=84, camera_name=None, segmentation=False):
